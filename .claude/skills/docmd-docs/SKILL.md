...
name: docmd-docs
description: >
  Author and maintain the public documentation site for laravel-iam-ai, built with docmd and living in
...
# docmd-docs — documentation site for laravel-iam-ai
...
**Accuracy first:** document what the package actually does — real classes (`AdvisoryClient`, `Advisory`,
`Modules\AccessExplainer`, `Governance\Redactor`, `Governance\HallucinationGuard`, `Contracts\AiProvider`,
`Providers\DisabledProvider`, `IamAiServiceProvider`), real config keys (`config/iam-ai.php`: `enabled`,
`provider`, `model`, `redaction`, `store_prompts`, `store_outputs`, `max_context_events`), the real audit
shape (`stream=ai`, `event_type=iam.ai.advisory`). The module is **advisory-only** (the PDP decides),
**sovereign-by-default** (off; Regolo EU / Ollama on-prem; never OpenAI), with **PRE-prompt redaction**, a
**hallucination guard**, and a **deterministic fallback** on every failure path. A wrong doc is worse than
none.
...
`.node-version` (20). The site is **doc.laravel-iam-ai.padosoft.com**.
